...
class Failed:
    ...
    def __init__(self, timestamp=time_stamp, application_name='app_name',
    ...
    @staticmethod
    def read_logfile():
        with open(path + '\\log\\log.csv', 'r') as log:
            for i in log:
                if 'Failed' in i:
                    print(i)

    def set_logdata(self, timestamp=time_stamp, application_name='app_name',
                    description='description', status='Failed', code='code'):
        failed_values = []
...
all_log = open(path + '\\log\\log.csv', 'w')
failed = Failed()
success = Success()
warning = Warning()
session = Session()
with open(path + "\\all_log.txt", "r", newline='') as log:
    write_file = csv.DictWriter(all_log, fieldnames=headers)
    write_file.writeheader()
    for i in log:
        if 'Failed' in i:
            write_file.writerows(failed.set_logdata(i[:19], i[43: 46],
                                                    i[50:i.index('[')],
                                                    code=i[i.index('['):i.index(']')+1]))
    log.seek(0)
    for j in log:
        if 'Success' in j:
            write_file.writerows(success.set_logdata(j[:19], j[43: 46], j[50:]))
    log.seek(0)
    for k in log:
        if 'Warning' in k and 'Failed' not in k:
            write_file.writerows(warning.set_logdata(k[:19], k[43: 46], k[50:]))
    log.seek(0)
    for l in log:
        if 'Session' in l:
            write_file.writerows(session.set_logdata(l[:19], l[43: 46], l[50:]))
all_log.close()
failed.read_logfile()
success.read_logfile()
warning.read_logfile()
session.read_logfile()
